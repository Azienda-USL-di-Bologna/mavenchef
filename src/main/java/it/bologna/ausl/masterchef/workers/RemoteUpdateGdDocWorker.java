...
//            entityBuilder.addTextBody(REQUEST_DESCRIPTOR_PART_NAME, requestData.toJSONString(), ContentType.APPLICATION_JSON);
            log.info(in.get("requestDescriptor") != null);
            ...
            try {
                ...
                String errorMessage = String.format("eccezione durante la chiamata all'url %s", servletUrl);
                log.error(errorMessage, ex);
                log.error(String.format("request-data: %s", requestData.toJSONString()));
                throw new WorkerException(getJobType(), errorMessage, new UpdateGdDocErrorDetails(ex.getStatusCode(), ex.getMessage()));
            }
