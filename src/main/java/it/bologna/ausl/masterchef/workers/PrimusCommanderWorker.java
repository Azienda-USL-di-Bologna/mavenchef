...
//            Properties config = Launcher.loadConfig(configFile);
            ...
            if (!StringUtils.isEmpty(config.getProperty("primuscommander.redis.port"))) 
                primusCommRedisPort=Integer.valueOf(config.getProperty("primuscommander.redis.port"));
            int primusCommRedisDB = 1;
            if (!StringUtils.isEmpty(config.getProperty("primuscommander.redis.db"))) 
                primusCommRedisDB=Integer.parseInt(config.getProperty("primuscommander.redis.db"));
//            int primusCommRedisMaxConn = Integer.valueOf(config.getProperty("primuscommander.redis.maxconnection", "4"));
            ...
            String redisPassword = config.getProperty("primuscommander.redis.password", null);
            ...
            RedisQueueManager redisPrimus = new RedisQueueManager(primusCommRedisUrl, primusCommRedisPort, primusCommRedisDB, Configuration.getRedisMaxConnection(), primusCommRedisCommandQueue, null, null, redisPassword);
            RedisQueueManager redis = new RedisQueueManager(Configuration.getRedisHost(), Configuration.getRedisPort(), Configuration.getRedisDB(), Configuration.getRedisMaxConnection(), Configuration.getInQueue(), Configuration.getWorkQueue(), Configuration.getOutQueue(), Configuration.getRedisPassword());
