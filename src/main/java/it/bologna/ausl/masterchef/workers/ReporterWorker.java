...
//                    itr.setPDFXConformance(org.openpdf.text.pdf.PdfWriter.PDFA1A);
                    org.openpdf.text.pdf.PdfWriter writer = itr.getWriter();
...
//                    writer.setPdfVersion(PdfWriter.PDF_VERSION_1_4);
                ...
                public void onClose(ITextRenderer itr) {
                    org.openpdf.text.pdf.PdfWriter writer = itr.getWriter();
                    try {
                        UtilityFunctions.writeExtraCatalog(writer, iccProfileStream);
                        UtilityFunctions.writeXmpMetadata(writer, iccProfileStream);               
                    }
                    catch (Exception ex) {
                        log.error(ex);
                    }
